...
Rectangle Soldier::GetRect() const
{
...
void Soldier::Update(const std::vector<Platform>& platforms)
{
    ...
    // Shoot when pressing C
    if (IsKeyPressed(KEY_C)) {
        Vector2 muzzle = { position.x + (facingRight ? (float)image.width : 0.0f), position.y + image.height * 0.5f };
        Shoot(muzzle, facingRight);
    }
...
void Soldier::ResolveShotsOnEnemies(std::vector<Enemy*>& enemies)
{
    ResolveHits(enemies);
}
